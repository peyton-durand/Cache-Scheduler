...
int main(int argc, char *argv[])
{
    ...
    fscanf(file_ptr, "%u", &TOTAL_CREATED_PROCESSES); // read the number of processes from the file
    for (int i = 0; i < TOTAL_CREATED_PROCESSES; i++) {
        fscanf(file_ptr, "%u %u %u %u", &process_list[i].A, &process_list[i].B, &process_list[i].C, &process_list[i].M);
